 ...
 */
(function ($, wp) {
	$('.default-percent-save').on('click', function (e) {
		e.preventDefault();
		let percent = $("[name='default-percent']").val();
		console.log(percent);

		let sendData = {
			'action': 'save_default_percent',
			'percent': percent,
			'_percent_nonce': $('#_percent_nonce').val(),
		}
		$('.default-percent-notice').hide();
		$('.default-percent-error').hide();
		$.ajax({
			url: ajaxurl,
			type: 'POST',
			data: sendData,
			...
			if (data.result == 'success') {
				...
				$('.default-percent-notice p').html(data.msg);
				$('.default-percent-notice').show();
				$("[name='default-percent']").val(data.val);
			} else {
				$('.default-percent-error p').html(data.msg);
				$('.default-percent-error').show();
	...
	window.inlineEditTax = {
		...
		edit: function (id) {
			...
			$(':input[name="name"]').css("width", val.length * 10);
			...
			$(':input[name="percent"]', editRow).val(val);
